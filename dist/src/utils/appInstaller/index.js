...
/* eslint-disable-next-line @typescript-eslint/explicit-function-return-type */
async function _installExternalApp({ name, version, network = 'mainnet', initializeArgs, skipInitialize, apmAddress, dao, ipfsGateway }, bre) {
    ...
    async function _publishApp(network, fullName, rootAccount, bre, version) {
        const networkConfig = bre.config.networks[network];
        const jsonRpcProvider = new ethers_1.ethers.providers.JsonRpcProvider(networkConfig.url, {
            name: network,
            chainId: networkConfig.chainId || 8545,
            ensAddress: networkConfig.ensAddress
        });
        ...
        const versionData = await getRepoVersion_1.default(fullName, version, jsonRpcProvider);
